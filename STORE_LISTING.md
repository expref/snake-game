...
| Asset | Spec | Status |
|---|---|---|
| **Hi-res icon** | 512 × 512 px, 32-bit PNG, ≤1 MB | ✅ Generated → `store/play-icon-512.png` |
| **Feature graphic** | 1024 × 500 px, PNG or JPG, no alpha | ✅ Generated → `store/feature-graphic.png` |
| **Phone screenshots** | 2–8 images, 9:16 (e.g. 1080 × 1920), 24-bit PNG/JPEG | ⬜ You capture (min 2): menu, gameplay, level reveal, win/celebration |
| **Tablet screenshots** | 7" and 10" sizes | ⬜ Optional but recommended |

> The icon + feature graphic are regenerated by `node scripts/gen-store-graphics.js`.
> Capture phone screenshots from a device/emulator running the `preview` (APK)
